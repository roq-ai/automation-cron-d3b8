...
import { getCronJobs } from 'apiSdk/cron-jobs';
...
import { CronJobListPage as ShowCaseList } from 'pages/cron-jobs';

function DashboardPage() {
  const { session } = useSession();

  const cronJobStat = useSWR<PaginatedInterface<CronJobInterface>>(
    () => `/cron-jobs`,
    () => getCronJobs({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Cron Jobs',
      result: cronJobStat?.data?.totalCount,
      isLoading: cronJobStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`cron-jobs`} passHref legacyBehavior>
                  <Button
